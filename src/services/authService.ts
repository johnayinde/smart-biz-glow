...
class AuthService {
  async login(credentials: LoginCredentials): Promise<{ data: AuthResponse | null; error: string | null }> {
  ...
  async signup(credentials: SignupCredentials): Promise<{ data: AuthResponse | null; error: string | null }> {
  ...
  async refreshToken(): Promise<{ data: AuthResponse | null; error: string | null }> {
    const refreshToken = localStorage.getItem('refresh_token');
    if (!refreshToken) {
